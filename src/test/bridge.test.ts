...
describe('MessageBridge', () => {
  it('on `ready` posts back sessions and messages for first session', async () => {
    ...
    const posts: unknown[] = [];
    const bridge = new MessageBridge({ api, post: m => posts.push(m), onRestartServer: vi.fn(), directory: '' });
  ...
  it('on `send` calls api.sendPrompt with text part and parsed model', async () => {
    const api = fakeApi();
    const bridge = new MessageBridge({ api, post: vi.fn(), onRestartServer: vi.fn(), directory: '' });
